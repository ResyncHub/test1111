...
export function JobCard({ job }: { job: Job }) {
  return (
    <Link href={`/jobs/${job.id}`}>
      <div
        className="rounded-xl border p-4 transition-colors active:opacity-80"
      ...
      >
        ...
        <div className="space-y-1">
          {job.customer && (
            <div className="flex items-center gap-1.5 text-xs" style={{ color: "hsl(215 20% 55%)" }}>
        ...
        {job.revenue > 0 && (
          <div className="mt-2 pt-2 border-t text-xs font-semibold" style={{ borderColor: "hsl(217 33% 18%)", color: "hsl(142 76% 46%)" }}>
