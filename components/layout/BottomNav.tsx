        ...
        {navItems.map(({ href, icon: Icon, label }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className="flex flex-col items-center justify-center gap-0.5 py-2 text-xs transition-all duration-150"
              style={{ color: active ? "hsl(217 91% 60%)" : "hsl(215 20% 55%)" }}
            >
              <div
                className="flex items-center justify-center rounded-xl w-10 h-6 transition-all duration-150"
